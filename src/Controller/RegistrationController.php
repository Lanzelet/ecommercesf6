...
class RegistrationController extends AbstractController
{
    ...
    EntityManagerInterface $entityManager, SendMailService $mail, JWTService $jwt): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $mail->send(
                'no-reply@monsite.fr',
                $user->getEmail(),
                'Activation de votre compte sur le site E-Commerce SF6',
                'register',
                compact('user', 'token')
            );
    ...
    {
       //On vérfie si le token, n'as pas expiré et n'a pas été modifié
       if($jwt->isValid($token) && $jwt->check($token, $this->getParameter('app.jwtsecret')) && !$jwt->isExpired($token)){
            ...
            //On vérifié que l'utilisateur existe et n'as pas encore active son compte
    ...
    public function resendVerif(JWTService $jwt, SendMailService $mail, UsersRepository $usersRepository): Response
    {
        ...
        if(!$user)
        {
            $this->addFlash('danger', 'Veuillez vous identifier');
            return $this->redirectToRoute('app_login');
        }
        ...
        $this->addFlash('success', 'Veuillez consulter votre boîte mail pour activer votre compte');
